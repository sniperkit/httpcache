...
// Package azurestorage provides an implementation of httpcache.Cache that
// stores and retrieves data using Azure Storage.
package azurestoragecache // import "github.com/PaulARoy/azurestorage"
...
func (c *Cache) Get(key string) (resp []byte, ok bool) {
	...
	if err != nil {
		if !noLogErrors {
			log.Printf("azurestorage.Get failed: %s", err)
		}
	}
...
func (c *Cache) Set(key string, value []byte) {
	...
	if err != nil {
		if !noLogErrors {
			log.Printf("azurestorage.Set failed: %s", err)
		}
		return
	}
}
...
func (c *Cache) Delete(key string) {
	...
	if !noLogErrors {
		log.Printf("azurestorage.Delete result: %s", res)
	}
	if err != nil {
		if !noLogErrors {
			log.Printf("azurestorage.Delete failed: %s", err)
		}
	}
}
